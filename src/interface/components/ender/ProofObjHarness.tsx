...
import { StaticAppPage, StaticAppPageProps } from "./StaticAppPage";
...
export interface ProofCheckSummary {
  parseSucceeded: boolean;
  incorrectSteps: string[];
  proofWideIssues: string[];
}

export interface ProofObjHarnessProps {
  /** Externally supplied proof text (e.g. a dataset proof). When set, the
   * bundled-proof selector is hidden and this text drives the editor. */
  proofText?: string;
  ...
  onCheckResult?: (result: ProofCheckSummary) => void;
}
...
export class ProofObjHarness extends Component<
  ProofObjHarnessProps,
  ProofObjHarnessState
> {
  ...
  constructor(props: ProofObjHarnessProps) {
    super(props);
    this.state = {
      isEditorOpen: props.proofText === undefined,
  ...
  private isControlledProof(): boolean {
    return this.props.proofText !== undefined;
  }

  componentDidMount(): void {
    if (!this.isControlledProof()) {
  ...
  componentDidUpdate(
    prevProps: ProofObjHarnessProps,
    prevState: ProofObjHarnessState,
  ): void {
    if (
      prevProps.proofText !== this.props.proofText &&
      this.props.proofText !== undefined
    ) {
      this.setState({ proofText: this.props.proofText });
    }
    ...
    this.proofParseTimeoutId = window.setTimeout(() => {
      ...
      try {
        ...
        this.setState(
          ...
          () => {
            this.props.onCheckResult?.({
              parseSucceeded: true,
              incorrectSteps: Array.from(result.graph.incorrectSteps).sort(),
              proofWideIssues: nextProofIssues,
            });
          },
        );
  ...
  private renderProofSelector(selectedProofKey: string): React.ReactNode {
    return (
      <select
        className="mb-2 border border-slate-300 rounded px-2 py-1 text-sm"
        value={selectedProofKey}
        onChange={(e) => this.setState({ selectedProofKey: e.target.value })}
      >
        {(() => {
      ...
      <div className="h-screen">
        <div className="sticky top-0 left-0 bg-slate-100 shadow-sm w-full p-3 z-30 flex justify-between">
          ...
          <div className="font-semibold">
            {this.props.title ?? "ProofObj Harness"}
          </div>
        ...
        {isEditorOpen && (
          ...
          >
            ...
            {!this.isControlledProof() &&
              this.renderProofSelector(selectedProofKey)}
