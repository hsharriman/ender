...
export const stmtToText =
  (stmt?: Stmt, ctx?: ProofContent) => (isActive: boolean) => {
    ...
    if (
      ...
      const centerLabel = ctx?.getCircle(args[0])?.center.label ?? args[0];
      return SegmentCircleClassification.text(
        args[1],
        centerLabel,
        stmt.function,
      )(isActive);
    ...
    if (stmt.function === "kite_premise" && args.length === 3) {
      return QuadClassification.text(args[0], "kite")(isActive);
    }
    if (stmt.function === "trapezoid_premise" && args.length === 3) {
      return QuadClassification.text(args[0], "trapezoid")(isActive);
    }
...
export const stmtListToText =
  (stmts: Array<Stmt | undefined>, ctx?: ProofContent) => (isActive: boolean) =>
    React.createElement(
      "span",
      null,
