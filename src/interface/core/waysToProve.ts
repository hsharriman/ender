...
/**
 * Restored from the legacy checker's `utils.ts`, which went with it: the only
 * thing this module needed from there.
 */
...
/**
 * Mini-figure candidates for a step, drawn from its conclusion's geometry:
 * for each way the reason's declared dependency slots can be filled from the
 * parts the conclusion names, one candidate the editor can draw.
 *
 * This reads the untrusted reason catalog and the display geometry, neither of
 * which the verified checker will touch, so it lives here rather than in the
 * Rocq report.  It suggests what to look at; it never decides anything.
 */
