...
const reasonDefs = `
reason given()
reason reflex() -> reflex, con_seg
reason sss(con_seg, con_ang, con_seg) -> con_tri
reason sss(con_seg, con_seg, con_seg) -> con_tri
reason asa(con_ang, con_seg, con_ang) -> con_tri
reason aas(con_ang, con_ang, con_seg) -> con_tri
reason rhl(con_right, con_seg, con_seg) -> con_tri
reason aaa(con_ang, con_ang, con_ang) -> sim_tri
reason cpctc(con_tri) -> con_seg, con_ang
reason vert_ang(intersect_seg) -> con_ang
reason conv_alt_int(para, transversal) -> con_ang
`;

const stmtDefs = `
stmt reflex(Segment s)
stmt right(Angle a) 

// congruence
stmt con_seg(Segment s1, Segment s2)
stmt con_ang(Angle a1, Angle a2)
stmt con_tri(Triangle t1, Triangle t2)
stmt con_right(Angle a1, Angle a2)

//similarity
// ratio of s1:s2 is the same as ratio of s3:s4
stmt sim_seg(Segment s1, Segment s2, Segment s3, Segment s4)
stmt sim_tri(Triangle t1, Triangle t2)

stmt para(Segment s1, Segment s2)
stmt perp(Segment s1, Segment s2)
stmt collinear(Point p1, Point p2, Point p3)
stmt midpoint(Segment s, Point p)

stmt intersect_seg(Segment s1, Segment s2, Point intersect)
stmt transversal(Segment s1, Segment s2, Segment s3)
`;
...
// test("parses a basic proof", () => {
//   const tree = parser.parse(proof);

//   // Check for top-level structure
//   expect(tree.topNode.getChild("Title")).not.toBeNull();
//   expect(tree.topNode.getChild("Premises")).not.toBeNull();
//   expect(tree.topNode.getChild("Steps")).not.toBeNull();
// });
